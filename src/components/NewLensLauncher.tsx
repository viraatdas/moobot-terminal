...
export function NewLensLauncher({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/45 px-4 pt-[12vh]">
      <div
        role="dialog"
        aria-modal="true"
        aria-label="New tab"
        className="w-full max-w-2xl overflow-hidden rounded-sm border border-hairline bg-bg shadow-2xl"
      >
        <div className="flex items-center justify-between border-b border-hairline bg-panel px-4 py-3">
          <div>
            ...
            <div className="mt-0.5 text-[13px] text-ink-dim">Choose a chat, template, or agent.</div>
        ...
        <div className="grid gap-px bg-hairline md:grid-cols-[1.05fr_1fr]">
          <div className="bg-bg p-4">
            <button
              type="button"
              onClick={() => void run("chat", onCreateChat)}
              disabled={busy !== null}
              className="group flex w-full items-start gap-3 rounded-sm border border-amber/40 bg-amber-dim px-3 py-3 text-left hover:bg-amber/25 disabled:opacity-50"
            >
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-sm border border-amber/35 bg-bg text-amber">
                {busy === "chat" ? (
              ...
              <span className="min-w-0">
                <span className="block text-[13px] font-semibold text-ink">Regular chat</span>
                <span className="mt-0.5 block text-[12px] leading-snug text-ink-dim">
                  Blank chat with the composer open and your data sources available.
                </span>
              </span>
            </button>

            <div className="mt-4 text-[10px] font-semibold tracking-[0.16em] text-ink-faint uppercase">
              Templates
            </div>
...
function LauncherRow({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="flex w-full items-center gap-2.5 rounded-sm border border-hairline bg-panel px-3 py-2 text-left hover:border-hairline-2 hover:bg-panel-2 disabled:opacity-50"
    >
      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-sm border border-hairline bg-bg text-ink-faint">
